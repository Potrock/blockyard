...
/**
 * High Noon on each player's screen: the platform's kits it uses, in the order they run (the
 * standard voices, the first-person view, the gunslingers' figures, the held gun's HUD, gunfire in
...
export default defineClient(shared, {
  kits: [...sounds.standard(), ...firstPerson.standard(), figures.humanoid(), hud.gunner(), effects.gunfire()],
  setup(client) {
    defineLooks(client);
    defineSounds(client);
  },
