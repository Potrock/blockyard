...
/**
 * A starfighter built from blocks. Ship space: the nose points to -z, up is +y, the right wing
 * is +x, and (0, 0, 0) is the centre of mass. One block is 1/4 m in game (see `SHIP_SCALE`).
 */
export interface ShipDesign {
  blueprint: Blueprint;
  /** Laser muzzles, in blocks (ship space). */
  guns: Vec3[];
  /** Engine exhaust points, in blocks (ship space), for glow and trails. */
  engines: Vec3[];
  /** Rough radius in blocks, for hits and collisions. */
  radius: number;
}
...
export function xwing(): ShipDesign {
  ...
  // Astromech droid dome behind the cockpit, sitting in a dark socket: white with blue panels,
...
export function tieInterceptor(): ShipDesign {
  ...
  for (const sx of [-1, 1])
    for (const sy of [-1, 1]) {
      const y = sy * 15;
      const x = sx * (xAt(15) - 1);
      bp.fill({ x, y, z: -13 }, { x, y, z: -5 }, (_x, _y, z) => (z <= -10 ? 'black_concrete' : 'gray_concrete'));
      guns.push({ x, y, z: -13.5 });
    }
  return { blueprint: bp, guns, engines: exhausts, radius: radiusOf(bp) };
}
