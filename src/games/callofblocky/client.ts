 ...
 * lethals and guns, fired and thrown here at once as its server plays them; the standard voices, the first-person view, the fighters' figures, the gun's and the lethals' HUD,
...
export default defineClient(shared, {
  kits: [items.throwables(), items.guns(), ...sounds.standard(), ...firstPerson.standard(), figures.humanoid(), hud.gunner(), hud.throwables(), effects.gunfire(), effects.throwables(), killcam(), progressionHud()],
  setup(client) {
    defineLooks(client);
    defineSounds(client);
  },
