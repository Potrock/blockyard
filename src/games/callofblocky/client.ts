 ...
 * gunfire and the lethals in the world, its kill cam on screen: `client/killcam.ts`, and what a
 * pilot sees flying a killstreak: `client/streaks.ts`), then its own look: each weapon's model,
 * icon, hold, tracer and trail (`client/looks.ts`), and its voices
...
export default defineClient(shared, {
  kits: [...sounds.standard(), ...firstPerson.standard(), figures.humanoid(), hud.gunner(), hud.throwables(), effects.gunfire(), effects.throwables(), killcam(), streakCam(), progressionHud()],
  setup(client) {
    defineLooks(client);
    defineSounds(client);
  },
