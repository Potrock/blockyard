import { defineServer, type Bot, type GameContext, type MenuHandle, type Pickup, type Player } from '@platform';
...
function loadoutMenu(game: GameContext, f: Fighter) {
  ...
  const entries = () =>
    ...
    })).map((e, i) => xp.gate(p, PRIMARIES[i], e)); // [progression] locked: greyed out, refused
  const lethals = () =>
    ...
    })).map((e, i) => xp.gate(p, Object.keys(LETHALS)[i], e)); // [progression]
...
function endMatch(game: GameContext, winner: Player | null) {
  ...
  xp.matchOver(table.map((f, i) => ({ player: f.player, place: i + 1, won: f.player === top }))); // [progression]
