...
export class Combat {
  ...
  update(dt: number, input: SimInput) {
    ...
    this.throwables(dt, input, def?.kind === 'throwable' ? stack!.item : null);
    if (def?.kind === 'gun' && stack) {
      this.drawing = false;
      this.charge = 0;
      // (Not while a throwable's being cooked.)
      this.gun(dt, this.cooking ? input.without(0) : input, stack.item, def);
      return;
    }
    if (!active) {
      this.drawing = false;
   ...
   */
  private throwables(dt: number, input: SimInput, inHand: string | null) {
    const sent = input.throws;
    if (sent) {
      for (const [serial, item, x, y, z, vx, vy, vz, cook] of sent) this.take(serial, item, { x, y, z }, { x: vx, y: vy, z: vz }, cook);
      return;
    }
    if (!input.active) {
      this.cooking = null;
  ...
  private take(serial: number, item: string, from: Vec3, v: Vec3, cook: number) {
    ...
    if (!isThrowable(def) || this.me.inventory.count(item) < 1 || this.me.now() - this.lastThrow < throwable(def).cooldown * 0.6) return this.me.refuse(key);
    const t = throwable(def);
