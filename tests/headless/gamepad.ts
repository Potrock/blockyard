...
export default function gamepad() {
  ...
  check(binds.Up === 'KeyL' && binds.R3 === 'Digit3' && binds.RB === 'KeyG' && binds.Down === null && binds.RT === 'LMB', `the game's buttons over the platform's: ${JSON.stringify(binds)}`);
  const hints = padHints(cob, true, { jump: 'Space', crouch: 'KeyC', sprint: 'ShiftLeft' });
  const said = Object.fromEntries(hints.map(([k, v]) => [v, k]));
  const want: Record<string, string> = { fire: 'RT', aim: 'LT', jump: 'A', 'crouch · slide': 'B', sprint: 'L3', reload: 'X', switch: 'LB Y', lethal: 'RB', katana: 'R3', loadout: 'D-pad ↑', scores: 'View', pause: 'Menu' };
