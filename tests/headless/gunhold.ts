...
import type { GunAction, HoldSpec, ViewAnimation } from '../../src/platform/api/types';
...
/**
 * How guns are held in first person, per item: a gun's points from its spec over its file's; one
 * hand or two (the view model's support hand out of sight but to reload); first-person arms fitted
 * per gun and bent at the elbow to reach a shoulder that stays put; a gun's action (a lever, a
 * hammer, one of the game's own) worked after each shot; two items sharing a model each keeping
 * its own hold. (How figures hold them: tests/headless/figures.ts.)
 */
export default async function gunhold() {
  ...
  console.log('  points from a spec over a file\'s · two items on one model held their own ways · one hand (hidden, in to reload) · lever, hammer, the game\'s own action · arms fitted per gun, bent to a shoulder that stays put, fists sized on their own');
}
