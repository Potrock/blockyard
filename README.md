...
- Movable block builds ("props", like the Starfighter ships): a Blueprint meshed once with the world's block textures, AO, shadows and glowing blocks, then moved freely every frame. glTF and GLB models (Blockbench, Blender) as props and as animated figures: idle, walk in step, run, attack, a head that looks, lit and shadowed like everything else. Glowing laser bolts, explosions (fireball, smoke, sparks, shockwave) and `world.explode` craters.
